...
-------------------------------------------------------------- */
  ...
  jQuery(".page-template-template-team-member .awards-slider").slick({
	...
	jQuery('.testimonial-mainn').slick({
  infinite: true,
  slidesToShow: 1,
  slidesToScroll: 1,
responsive: [
...
/**Per Client Request
 * Do not load Messenger on Page ID 326045
 * Check to see if the Page ID exists as a Class
 * If its there, dont load Ngage. Otherwise load it.
 */
const pageId = document.querySelector('.page-id-326045');
if ( pageId === null ){
    delayScript(
        "https://messenger.ngageics.com/ilnksrvr.aspx?websiteid=244-114-91-248-145-38-209-6",5000
    );
}
